...
def run(use_internet: str):
    ...
    def install_package(package_input: str, output: bool = True, is_run: bool = False):
        ...
        if output:
            ...
            install_sderr = install_process.stderr.split('\n')
            if install_sderr[0][:31] == "ERROR: Could not find a version" or install_sderr[0][:27] == "ERROR: Invalid requirement:":
            ...
            elif install_sderr[0][:111] == "WARNING: Retrying (Retry(total=4, connect=None, read=None, redirect=None, status=None)) after connection broken":
                ...
                if 1 < len(package) < 3:
                    ...
                    uprade_process = subprocess.run([sys.executable, "-m", "pip", "install", "--upgrade", package[0]],
    ...
    def generator_ru_words():
        ...
        def gen_value():
            def generator_process(all_of_letters: str, length_of_words: int):
                ...
                #    print("ValueError: You need to write correct mode (Fast or Slow)")
                #    return
                #
                ...
                with open('russian_nouns_without_io.txt', encoding='utf-8') as f1:
                    ...
                    result.append(f"Слова из {length_of_words} букв:\n")
                    ...
                    for word in words:
                        ...
                        for j in range(len(list_of_ru_words)):
                            if generate_word == list_of_ru_words[j] and count == 0:
                                result.append(f"{count_2} слово: {generate_word}\n")
                                count += 1
                                count_2 += 1
